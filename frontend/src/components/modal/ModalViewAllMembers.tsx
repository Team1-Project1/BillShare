...
export default function ModalViewAllMembers({
  isOpen,
  onClose,
  ...
  if (!userId) {
    ...
    // SỬA: Thêm return null để tránh lỗi render
    return null;
  }

  const handleSelectMember = (id: number) => {
    if (id === userId) {
      toast.warn("Bạn không thể chọn chính mình để xóa!", {
        position: "top-center",
      });
      return;
    }
  ...
  const handleDeleteMembers = async () => {
    try {
      // SỬA: Xóa 'results =' không dùng đến
      await Promise.all(
